...
//
// Env vars:
//   BASE_URL   default http://localhost:8080 — this is only a fallback for the .env.example
//              default; k6 does not read .env itself, so pass BASE_URL explicitly if SERVER_PORT
//              differs (it does by default in this repo's own .env — see render-prometheus-config.sh)
//   VUS        default 20   (concurrent virtual users)
//   DURATION   default 2m   (steady-state duration, after ramp-up)
//   POOL_SIZE  default 0    (0 = use every seeded conversation id; set smaller, e.g. 50, to
//                            concentrate traffic on a small "hot" set — mimics active users and
//                            keeps the Redis cache warm within its 30 min TTL)
